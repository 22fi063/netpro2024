...
class GUIAnimatinFaceLook {// 顔のオブジェクト
	...
	public void makeBrow(Graphics g, int emotionLevel) {
		int xLeft = xStart + h / 5; // 左眉の開始位置
		int yTop = yStart + w / 4; // 眉の開始位置（高さ）
		int browWidth = h/5; // 眉の幅
		int browHeight = 10; // 眉の高さ調整

		if (emotionLevel == 0) {
			// 通常の眉
			g.drawLine(xLeft, yTop, xLeft + browWidth, yTop);
			g.drawLine(xLeft + browWidth * 2, yTop, xLeft + browWidth * 3, yTop);
		} else if (emotionLevel == 1) {
			// 怒っている眉
			g.drawLine(xLeft, yTop + browHeight, xLeft + browWidth, yTop - browHeight);
			g.drawLine(xLeft + browWidth * 2, yTop - browHeight, xLeft + browWidth * 3, yTop + browHeight);
		} else {
			// 驚いた眉
			g.drawLine(xLeft, yTop - browHeight, xLeft + browWidth, yTop + browHeight);
			g.drawLine(xLeft + browWidth * 2, yTop + browHeight, xLeft + browWidth * 3, yTop - browHeight);
